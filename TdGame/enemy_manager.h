...
private:
	...
	void process_bullet_collision()
	{
		...
		for (Enemy* enemy : enemy_list)
		{
			...
			const Vector2 pos_enenmy = enemy->get_position();

			for (Bullet* bullet : bullet_list)
			{
				if (!bullet->can_collide()) continue;

				const Vector2 pos_bullet = bullet->get_position();  // 小的中心点进入大的就算碰撞

				if (pos_bullet.x >= pos_enenmy.x - size_enemy.x / 2
					&& pos_bullet.y >= pos_enenmy.y - size_enemy.x / 2
					&& pos_bullet.x <= pos_enenmy.x + size_enemy.x / 2
					&& pos_bullet.y <= pos_enenmy.y + size_enemy.y / 2)
				{
					double damage = bullet->get_damage();
					double damage_range = bullet->get_damage_range();
					if (damage_range < 0)
					{
						//std::cout << "Damge: " << damage << std::endl;
						//std::cout << "HP: " << enemy->get_hp() << std::endl;

						enemy->decrease_hp(damage);
						if (enemy->can_remove())
						{
							try_spawn_coin_prop(pos_enenmy,enemy->get_reward_ratio());
						}
					}
					else
					{
						for (Enemy* target_enemy : enemy_list)
						{
							...
							if ((pos_target_enemy - pos_bullet).length() <= bullet->get_damage_range())
							{
	...
	void try_spawn_coin_prop(const Vector2& pos, double ratio)
	{
		static CoinManager* instance = CoinManager::instance();

		if ((double)(rand() % 100) / 100 <= ratio)
			instance->spawn_coin_prop(pos);
	}
};
