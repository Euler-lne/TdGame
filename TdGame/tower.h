...
private:
	...
	void on_fire()
	{
		...
		double interval = 0, damage = 0;  // 更新对应的参数
		switch (tower_type)
		{
		case Archer:
		...
		timer_fire.set_wait_time(interval);   // 设置冷却
		timer_fire.restart();

		Vector2 direction = (enemy->get_position() - position).normalize();
		bool is_show_x_anim = abs(direction.x) >= abs(direction.y);  // 调整位置
		if (is_show_x_anim)
		...
		BulletManager::instance()->fire_bullet(bullet_type, position, direction * fire_speed * SIZE_TILE, damage); // 开火
