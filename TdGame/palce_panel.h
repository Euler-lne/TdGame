...
public:
	...
	void on_update(SDL_Renderer* renderer) override
	{
		...
		reg_top = (int)instance->get_damager_range(TowerType::Axeman) * SIZE_TILE; // 转换为世界坐标
		reg_left = (int)instance->get_damager_range(TowerType::Archer) * SIZE_TILE;
		reg_right = (int)instance->get_damager_range(TowerType::Gunner) * SIZE_TILE;
	...
	void on_render(SDL_Renderer* renderer) override
	{
		...
		if (reg > 0)
		{
			filledCircleRGBA(renderer, center_pos.x, center_pos.y, reg,
				color_redion_content.r, color_redion_content.g, color_redion_content.b, color_redion_content.a);
...
private:
	...
	const SDL_Color color_redion_content = { 0,149,217,175 };
