...
public:
	...
	{
		std::string window_tile = u8"村庄保卫战";
	...
	struct EnemyTemplate
	{
		...
		double recover_intensity = 25; // 回复强度
	};
...
public:
	bool load_level_config(const std::string& path)
	{
		...
		if (json_root->type != cJSON_Array) { // 最外层要是数组
		...
		cJSON_ArrayForEach(json_wave, json_root)
		{
			...
			if (json_wave_spawn_list && json_wave_spawn_list->type == cJSON_Array) 
			{
				...
				cJSON_ArrayForEach(json_spawn_event, json_wave_spawn_list)
				{
					...
					cJSON* json_spawn_event_enemy = cJSON_GetObjectItem(json_spawn_event, "enemy");
					if (json_spawn_event_enemy && json_spawn_event_enemy->type == cJSON_String)
					{
						 const std::string str_enemy_type = json_spawn_event_enemy->valuestring;
				...
				if (wave.spawn_event_list.empty()) // 没有解析出任何额波次列表，就要将这波删除
					wave_list.pop_back();
			}
		}
	...
	bool load_game_config(const std::string& path)
	{
		...
		cJSON* json_basic, * json_player, * json_tower, * json_enemy;
		json_basic = cJSON_GetObjectItem(json_root, "basic");
		json_player = cJSON_GetObjectItem(json_root, "player");
		json_tower = cJSON_GetObjectItem(json_root, "tower");
		json_enemy = cJSON_GetObjectItem(json_root, "enemy");
		...
		parse_tower_template(archer_template, cJSON_GetObjectItem(json_tower, "archer")); // 直接写cJSON_GetObjectItem 因为函数中判断好了
...
private:
	void parse_basic_template(BasicTemplate& tpl, cJSON* json_root) 
	{
		if (!json_root || json_root->type != cJSON_Object)
			return;
		cJSON* json_window_title, * json_window_width, * json_window_height;
		json_window_title = cJSON_GetObjectItem(json_root, "window_title");
		if (json_window_title && json_window_title->type == cJSON_String)
			tpl.window_tile = json_window_title->valuestring;

		json_window_width = cJSON_GetObjectItem(json_root, "window_width");
		if (json_window_width && json_window_width->type == cJSON_String)
			tpl.window_width = json_window_width->valueint;

		json_window_height = cJSON_GetObjectItem(json_root, "window_height");
	...
	void parse_player_template(PlayerTemplate& tpl, cJSON* json_root)
	{
		if (!json_root || json_root->type != cJSON_Object)
			return;
		cJSON* json_speed, * json_normal_attack_interval, * json_normal_attack_damage;
		cJSON* json_skill_interval,*json_skill_damage;

		json_speed = cJSON_GetObjectItem(json_root, "speed");
		json_normal_attack_interval = cJSON_GetObjectItem(json_root, "normal_attack_interval");
		json_normal_attack_damage = cJSON_GetObjectItem(json_root, "normal_attack_damage");
		json_skill_interval = cJSON_GetObjectItem(json_root, "skill_interval");
		json_skill_damage = cJSON_GetObjectItem(json_root, "skill_damage");
	...
	void parse_tower_template(TowerTemplate& tpl, cJSON* json_root)
	{
		if (!json_root || json_root->type != cJSON_Object)
			return;
		cJSON* json_interval, * json_damage, * json_view_range,*json_cost,*json_upgrade_cost;

		json_interval = cJSON_GetObjectItem(json_root, "interval");
		json_damage = cJSON_GetObjectItem(json_root, "damage");
		json_view_range = cJSON_GetObjectItem(json_root, "view_range");
		json_cost = cJSON_GetObjectItem(json_root, "cost");
		json_upgrade_cost = cJSON_GetObjectItem(json_root, "upgrade_cost");
	...
	void parse_enemy_template(EnemyTemplate& tpl, cJSON* json_root)
	{
		if (!json_root || json_root->type != cJSON_Object)
			return;
		cJSON* json_hp, * json_speed, * json_damage, * json_reward_ratio, * json_recover_interval;
		cJSON* json_recover_range, * json_recover_intensity;

		json_hp = cJSON_GetObjectItem(json_root, "hp");
		json_speed = cJSON_GetObjectItem(json_root, "speed");
		json_damage = cJSON_GetObjectItem(json_root, "damage");
		json_reward_ratio = cJSON_GetObjectItem(json_root, "reward_ratio");
		json_recover_interval = cJSON_GetObjectItem(json_root, "recover_interval");
		json_recover_range = cJSON_GetObjectItem(json_root, "recover_range");
		json_recover_intensity = cJSON_GetObjectItem(json_root, "recover_intensity");
